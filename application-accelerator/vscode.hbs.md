...
> **Note** If the language features are not available after opening an `accelerator.axl` file, you
> might have to manually activate the Application Accelerator VS Code extension. To do so, click the
> Tanzu Application Accelerator extension icon in the Activity Bar, and then re-open the
> `accelerator.axl` file.
